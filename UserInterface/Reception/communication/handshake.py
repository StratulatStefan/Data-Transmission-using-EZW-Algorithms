...
def CommunicationHandshake(printer,sock, data) -> bool:
	...
	if invalidcomtypeId or not ConnectionCheck(printer,connection, comtypeId):
...
def ConnectionCheck(printer,connection, selection) -> bool:
	...
	while True:
		...
		printer(f">>> Am primit mesajul {decoded_data}")
		...
		#	pass
			...
			ack = f"[HS] ACK MSG{id}"
            		# trimitem confirmare
			printer(f"* Trimitem mesajul de confirmare pentru MSG{id}")
			writeFunction(connection, ack)
			printer(f">>> Am trimis confirmare pentru MSG{id}!")
			if id == 3:
				# am primit cele trei mesaje, deci handshake-ul a fost efectuat cu succes!
				break
	printer(f"* Handshake realizat cu succes!")
	return True
